    ...
    <div className="page-container">
      ...
      <div className="calendar-controls">
        <div className="calendar-nav">
          <button className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem' }} onClick={handlePrevMonth}>
            &larr;
          </button>
          <h2>{viewMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h2>
          <button className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem' }} onClick={handleNextMonth}>
            &rarr;
          </button>
        </div>
        ...
        <div className="calendar-list-view">
          {filteredEvents.length === 0 ? (
            <div className="empty-state">
              ...
              <button className="btn btn-primary" onClick={() => openAddForm(formatDateString(new Date()))}>
                + Add Event
              </button>
            </div>
          ) : (
          ...
          {filteredEvents.length > 0 && (
            <button className="btn btn-primary" style={{ alignSelf: 'flex-start', marginTop: '1rem' }} onClick={() => openAddForm(formatDateString(new Date()))}>
              + Add Event
            </button>
          )}
        </div>
